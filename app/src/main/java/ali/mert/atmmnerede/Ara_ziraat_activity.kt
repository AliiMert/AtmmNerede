...
import ali.mert.atmmnerede.databinding.LayoutAramaZiraatBinding
import android.R
...
class Ara_ziraat_activity : ComponentActivity(), AdapterView.OnItemClickListener {
    private lateinit var checkNetworkConnection: InternetConnection //internet bağlantısı
    lateinit var binding : LayoutAramaZiraatBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = LayoutAramaZiraatBinding.inflate(layoutInflater)
        setContentView(binding.root)
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true){
            override fun handleOnBackPressed() {
                val intent = Intent(this@Ara_ziraat_activity, BankalarListesi_activity::class.java)
                startActivity(intent)
                finish()
            }
        })

        binding.listviewZiraat?.choiceMode = ListView.CHOICE_MODE_SINGLE
        binding.listviewZiraat?.onItemClickListener = this
        ...
        try {
            ...
            call?.enqueue(object : Callback<List<PostModel_banka?>?>{
                override fun onResponse(
                    ...
                    binding.spinnerZiraatIl.adapter = adapter
                }
        ...
        binding.spinnerZiraatIl.onItemSelectedListener = object :
            AdapterView.OnItemSelectedListener{
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                try {
                    var secilenilisim = binding.spinnerZiraatIl.selectedItem
                    ...
                    var rf2 = Retrofit.Builder()
                        ...
                        .addConverterFactory(GsonConverterFactory.create())
                        .build()
                    var API2 = rf2.create(RetrofitInterface_ziraat::class.java)
                    var call2 = API2.post
        ...
        binding.spinnerZiraatIlce.onItemSelectedListener = object :
            AdapterView.OnItemSelectedListener{
            override fun onItemSelected(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
                try {
                    var secilenilisim = binding.spinnerZiraatIl.selectedItem
                    var secilenil = PlakaToCity.reverseMap[secilenilisim]
                    var secilenilce = binding.spinnerZiraatIlce.selectedItem

                    var rf3 = Retrofit.Builder()
                        .baseUrl(RetrofitInterface_ziraat.BASE_URL)
                        .addConverterFactory(GsonConverterFactory.create())
                        .build()
                    var API3 = rf3.create(RetrofitInterface_ziraat::class.java)
                    var call3 = API3.post
        ...
        binding.buttonZiraatEnyakinatm.setOnClickListener(){
            try {
                    ...
                    call?.enqueue(object : Callback<List<PostModel_banka?>?> {
                        override fun onResponse(
                            ...
                            nearestATM?.let {
                                ...
                                val intent = Intent(this@Ara_ziraat_activity, Bilgi_ziraat_activity::class.java)
    ...
    private fun getLocation() {
        try {
            ...
            if (ContextCompat.checkSelfPermission(this, android.Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
                ...
                //erişilen konum bilgileri değişkenlere atandı
                if (location != null) {
                    val latitude = location.latitude
                    val longitude = location.longitude
                //erişilen enlem ve boylam bilgileri globalk değişkenelere atandı
    ...
    //listview’de bir nesneye tıklandığında
    override fun onItemClick(p0: AdapterView<*>?, p1: View?, p2: Int, p3: Long) {
        try {
            //tıklanan nesnenin bilgileri secilensube değişkenine atandı
            var secilensube : String = p0?.getItemAtPosition(p2) as String
            arananilisim = binding.spinnerZiraatIl.selectedItem.toString()
            var arananil = PlakaToCity.reverseMap[arananilisim]
            arananilce = binding.spinnerZiraatIlce.selectedItem.toString()

            //bilgilerin gösterileceği ekranın açılması için intent oluşturuldu
            val intent = Intent(this@Ara_ziraat_activity, Bilgi_ziraat_activity::class.java)
            //intente ekstra olarak seçilen şubenin bilgileri
            intent.putExtra("secilensube",secilensube)
            intent.putExtra("arananil", arananil)
            intent.putExtra("arananilce", arananilce)
            //sayfa değişmesi gerçekleştirildi
            startActivity(intent)
            finish()
        } catch (e: Exception) {
            Toast.makeText(
